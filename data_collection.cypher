...
MATCH (ath:Athlete)-[:HAS_PARTICIPANT*2]->(partInLot:Participant)-[:INVOLVED_IN]->(lot)<-[:HAS_SUBSTRUC*1..4]-(season:Competition {compStructureTerm:"Season"})
OPTIONAL MATCH (partInLot)-[:HAS_ROLE]->(position:Role)
RETURN ath.entID AS playerID, ath.nameLastF AS playerName, season.startDate.year AS year, position.termKRWebsite AS playerRole, numBids, price
ORDER BY playerID, year, playerRole, numBids

//PlayerBidSequence
MATCH (ath:Athlete{entID:60703})-[:HAS_PARTICIPANT*2]->(partInLot:Participant)-[:INVOLVED_IN]->(lot:Competition {compStructureTerm: "Auction lot"})<-[:HAS_SUBSTRUC*1..4]-(season:Competition {compStructureTerm:"Season"})OPTIONAL MATCH (partInLot)-[:HAS_ROLE]->(position:Role)
WITH DISTINCT season, lot, ath, position
MATCH p1 = (lot)-[:HAS_EVENT]->(bid)WHERE (exists ( (bid)-[:TYPE_OF]->(:Action {actID:3})) OR exists( (bid)-[:TYPE_OF]->(:Action {actID:4}) ))  AND NOT exists ( (bid)-[:TYPE_OF]->(:Action {actID:2}) ) // Filter out lots with only a base price bid
...
RETURN DISTINCT bid.seqOrder AS bidNum,ath.entID AS playerID, ath.nameLastF AS playerName,season.startDate.year AS year,position.termKRWebsite AS playerRole,team.termKRWebsiteLong AS teamName
...
CALL {
	...
	WHERE a.actID = 3 OR a.actID = 4
	WITH bid
	MATCH (lot:Competition {compStructureTerm: "Auction lot"})-[:HAS_EVENT]->(bid)<-[:INVOLVED_IN]-(:SportSquad)<-[:HAS_SQUAD*2]-(team:SportTeam)
	...
	WITH bid
	MATCH (lot:Competition {compStructureTerm: "Auction lot"})-[:HAS_EVENT]->(bid)<-[:INVOLVED_IN]-(:SportSquad)<-[:HAS_SQUAD*2]-(team:SportTeam)
	WHERE team.termKRWebsite IS NOT NULL AND (team.termKRWebsite = "Gujarat Titans" OR team.termKRWebsite = "Super Giants") AND bid.seqOrder > 0
